...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__project_home__ = __webpack_require__(484);
...
var ProjectHomePage = /** @class */ (function () {
    ...
    ProjectHomePage.prototype.ionViewDidLoad = function () {
        ...
        this.httpCall.callApi(requestJson, this.codes.API_GET_USER_WISE_PROJECT_REPORT).then(function (responseJson) {
            ...
            for (var i = 0; i < _this.projects.length; i++) {
                ...
                _this.projects[i]['materialIssueListShow'] = false;
            ...
            _this.httpCall.callApi(reqJson, _this.codes.API_GET_MATERIAL_REQUISITION_LIST).then(function (respJson) {
                ...
                _this.httpCall.callApi(reqJson, _this.codes.API_LIST_ISSUE_MATERIAL).then(function (matRespJson) {
                    if (_this.dataValidation.isEmptyJson(matRespJson)) {
                        _this.msgHelper.showErrorDialog('Error !!', 'Empty response received from server (Get Material Issue List) !!!');
                        return;
                    }
                    _this.materialIssues = matRespJson['resultData'];
                    console.warn('Issue Materials : ' + JSON.stringify(_this.materialIssues));
                    //Update the projects
                    for (var i = 0; i < _this.projects.length; i++) {
                        var material = _this.getMaterialByProjectId(_this.projects[i]['ProjectId'], _this.materials);
                        var materialIssueList = _this.getMaterialIssueByProjectId(_this.projects[i]['ProjectId'], _this.materialIssues);
                        if (!_this.dataValidation.isEmptyJson(material)) {
                            _this.projects[i]['materials'] = material;
                        }
                        if (!_this.dataValidation.isEmptyJson(materialIssueList)) {
                            _this.projects[i]['materialIssues'] = materialIssueList;
                        }
                    }
                });
            });
    ...
    ProjectHomePage.prototype.refundMaterial = function (materialIssue) {
        var materialModal = this.modalCtrl.create('MaterialRefundPage', { 'material': materialIssue });
        materialModal.present();
    };
    ProjectHomePage.prototype.getMaterialIssueByProjectId = function (projectId, materialList) {
        var material = [];
